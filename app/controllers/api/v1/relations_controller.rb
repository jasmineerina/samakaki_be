class Api::V1::RelationsController < ApplicationController
  ...
  def create_notif_invited_user
    @token= JWT.decode(params[:token_invitation], 'secret')[0]
    @relation = Relation.find_by_id(@token["relation_id"])
    @notif = Notification.new(user_relation_id:@relation.user_relations[0].user_id,user_id:@user.id,status:0,descriptions:"Anda di invit ke dalam family tree oleh #{@relation.user_relations[0].user.name}, apakah anda mengenal #{@relation.user_relations[0].user.name}?")
