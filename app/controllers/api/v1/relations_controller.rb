class Api::V1::RelationsController < ApplicationController
  ...
  before_action :verif_email, except: [:create_notif_invited_user]
  def create
  ...
  def update
    ...
    @relation.update(relation_params) ? response_to_json({relation: @relation,message:"berhasil memperbarui hubungan"}, :success) : response_error(@relation.errors,:unprocessable_entity)
  end
  ...
  def create_notif_invited_user
    begin
      ...
      @notif = Notification.find_or_create_by(user_relation_id:@relation.user_relations[0].id,user_id:@user.id,status:0,descriptions:"Anda di undang ke dalam family tree oleh #{@relation.user_relations[0].user.name}, apakah anda mengenal #{@relation.user_relations[0].user.name}?")
